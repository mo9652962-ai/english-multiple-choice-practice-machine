...
### 学习闭环与跨端发布
- 补齐诊断 → 推荐 → 针对性练习 → 错题/词汇 → FSRS 复习的可执行链路，并在今日计划中展示推荐原因、预期效果和证据状态。
- 增加 ESQ 题包来源/授权/人工复核/AI diff/发布抽样证据模型；结构质量检查与公开发布门禁分离，未完成证据的题包保持 `pending`。
- 统一程序版本、正式内容版本和离线种子版本，发布清单记录数据库计数、Schema、SHA-256 和构建产物指纹。
- 增加离线旧库/新库迁移回归、前端包体门禁、Web/Windows/Android smoke 流程。
- Android CI 增加触控、键盘、旋转、断网启动和证据上传；完成 JDK 21/SDK 36/Gradle Debug 构建流程。

### AI 与隐私
- AI 任务统一经过任务路由、结构化输出、超时/重试、缓存、配额和无模型降级；缓存命中不重复消耗配额。
- 本地学习指标默认关闭，用户明确同意后才记录 allow-list 事件，不记录题目正文、答案或 API Key。

### 发布说明
- 当前两个正式题包仍缺少完整公开发布证据，不能直接宣称为可公开分发内容。
- 当前 Android 产物为 Debug APK；正式签名和至少一台真实设备验收仍待完成。
