...
### 🚀 架构升维与多端基准
- **版本与全端发布统一**：版本号正式推进至 `v2.2.0`（2026-09-23），统一同步 Windows NSIS 安装包、Portable 便携版与 Android APK 发布清单与 SHA-256 指纹。
- **3D 水墨沉浸式官网生态**：上线 3D WebGL 交互官网（集成 3D 词汇磁斥力场、古卷爆炸解构、键盘极速做题工坊与真题星系罗盘）。
- **用户隔离与组织工作区健全**：深度完善组织工作区（Organization Workspace）、动态组卷模式（按题型×题量精准抽题）、防作弊行为审计与考试证书防伪验证。
- **题库格式与隐私合规标准**：巩固 ESQ 1.1 题库数据契约，严格物理隔离项目内置 AI 模拟题与用户私有题库，敏感凭据全量采用 Windows DPAPI / Android Keystore 硬件级加密。
