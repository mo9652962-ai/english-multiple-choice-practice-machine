...
const positiveNumber = (raw, flag) => {
  ...
  if (!Number.isFinite(number) || number <= 0) {
    throw new Error(`${flag} 必须是大于 0 的数字`)
  }
  return number
}

const maxEntryBytes = positiveNumber(valueFor('--max-entry-kb', '512'), '--max-entry-kb') * 1024
const maxLazyValue = valueFor('--max-lazy-kb', '')
const maxLazyBytes = maxLazyValue === ''
  ? null
  : positiveNumber(maxLazyValue, '--max-lazy-kb') * 1024
