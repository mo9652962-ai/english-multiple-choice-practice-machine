...
项目当前处于 `v2.0.0-beta.15 / 持续开发阶段`。核心刷题与复习链路已经可用，移动端、AI 辅助学习、题库分享、安全存储和自动更新等能力正在持续打磨；更多考试模板、便携发布包和更完整的多用户隔离仍在完善中。公开 GitHub Actions CI 已覆盖前端类型检查/构建、后端单元测试和后端导入检查。
