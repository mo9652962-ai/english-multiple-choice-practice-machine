...
def get_consent(connection: sqlite3.Connection) -> bool:
...
def set_consent(connection: sqlite3.Connection, enabled: bool) -> bool:
    connection.execute(
    ...
    if not enabled:
        connection.execute("DELETE FROM local_metrics_events")
    connection.commit()
    return enabled
...
def record_event(
    ...
    *,
    ...
    if event_name not in EVENT_NAMES or not get_consent(connection):
        return False
    safe_detail = {
...
def summarize(
    ...
    *,
    ...
    return {
        "days": days,
        "enabled": get_consent(connection),
        "total_events": total,
