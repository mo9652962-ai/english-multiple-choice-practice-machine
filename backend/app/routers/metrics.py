...
@router.get("/consent")
def read_consent(connection: sqlite3.Connection = Depends(get_db)) -> dict[str, bool]:
    return {"enabled": get_consent(connection)}
...
def update_consent(
    ...
    return {"enabled": set_consent(connection, request.enabled)}
