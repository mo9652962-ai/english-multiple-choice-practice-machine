...
class ReleaseContentQualityTests(unittest.TestCase):
    def test_paper_provenance_requires_every_active_paper_to_use_publishable_package(self) -> None:
        ...
        try:
            ...
            manifest = {
                "license": {
                    "notice": "Project-generated content",
                    "spdx": "CC0-1.0",
                    "verified": True,
                },
                "source": {"description": "Generated by the repository", "verified": True},
                "review": {"status": "reviewed"},
                "ai_assist": {"diff_status": "recorded"},
                "quality": {"release_sample": {"status": "passed"}},
            }
            connection.execute(
                "INSERT INTO question_bank_packages VALUES (?, ?, ?)",
                ("owned.package", "2026-r1", json.dumps(manifest)),
            )
            connection.executemany(
                "INSERT INTO papers VALUES (?, ?, ?, 'published', NULL, ?, ?)",
                [
                    (1, 2026, "Owned package paper", "owned.package", "2026-r1"),
                    (2, 2026, "Unmapped paper", None, None),
                    (3, 2026, "Deleted paper", None, None),
                ],
            )
            connection.execute("UPDATE papers SET deleted_at = '2026-09-14' WHERE id = 3")
            connection.commit()

            tables = {"papers", "question_bank_packages"}
            package_report = _package_provenance(connection, tables)
            result = _paper_provenance(connection, tables, package_report)

            self.assertEqual(result["papers_total"], 2)
            self.assertEqual(result["papers_with_publishable_package"], 1)
            self.assertEqual(result["papers_not_publishable"], 1)
            self.assertEqual(result["missing"][0]["paper_id"], 2)
            self.assertIn("package_id", result["missing"][0]["missing"])
        finally:
            connection.close()
